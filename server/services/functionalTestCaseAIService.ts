...
/** 用例类型排序权重（与前端 TestCases 九类一致） */
const CASE_TYPE_SORT_ORDER: Record<string, number> = {
  SMOKE: 1,
  FULL: 2,
  BOUNDARY: 3,
  ABNORMAL: 4,
...
const CASE_TYPE_JSON_ENUM =
  'SMOKE|FULL|ABNORMAL|BOUNDARY|PERFORMANCE|SECURITY|USABILITY|COMPATIBILITY|RELIABILITY';
...
export interface TestPoint {
  ...
  testPointType?: 'main' | 'abnormal' | 'boundary' | 'permission' | 'state' | 'security' | 'regression'; // 测试点类型
  coveredRequirementRefs?: string[]; // 关联需求编号，如 ["FR-C-001", "FR-009"]
  source?: 'requirement' | 'supplemental'; // 测试点来源：需求明确 / 规则补充
  sourceLabel?: string; // 来源展示文案
  sourceReason?: string; // 补充依据说明
  steps: string; // 测试步骤
...
export interface TestCase {
  ...
  coverageAreas?: string; // 覆盖范围
  caseType?: string; // 用例类型: SMOKE | FULL | ABNORMAL | BOUNDARY | PERFORMANCE | SECURITY | USABILITY | COMPATIBILITY | RELIABILITY
  // 兼容性字段
...
export class FunctionalTestCaseAIService {
  ...
  private getReasoningContent(message: any): string {
  ...
  private getReasoningTokenCount(usage: any): number {
    const tokenCount = usage?.completion_tokens_details?.reasoning_tokens
      ?? usage?.output_tokens_details?.reasoning_tokens
      ?? usage?.reasoning_tokens;
    return Number.isFinite(tokenCount) ? Number(tokenCount) : 0;
  }

  private resolveScenarioAnalysisMaxTokens(config: LLMConfig, isLargeRequirementDoc: boolean): number {
    const envOverride = parseInt(
      process.env.FUNCTIONAL_SCENARIO_MAX_OUTPUT_TOKENS
        || process.env.FUNCTIONAL_TEST_SCENARIO_MAX_TOKENS
        || '',
      10
    );
    if (Number.isFinite(envOverride) && envOverride >= 4000) {
      return envOverride;
    }

    const configured = Math.max(4000, Math.floor(config.maxTokens || 4000));
    const model = (config.model || '').toLowerCase();
    const isReasoningHeavyModel =
      model.includes('deepseek-v4')
      || model.includes('deepseek-reasoner')
      || model.includes('deepseek-r1')
      || model.includes('reasoner')
      || /\br1\b/.test(model);
    const floor = isLargeRequirementDoc ? 16000 : (isReasoningHeavyModel ? 12000 : 6000);
  ...
  private async callAIInternal(systemPrompt: string, userPrompt: string, maxTokens?: number): Promise<string> {
    ...
    // 🔥 仅验证 maxTokens 基本格式，不再用本地固定值裁剪；实际上限交给模型/API 提供商处理
    const finalMaxTokens = Math.max(1, Math.floor(maxTokens ?? config.maxTokens));
   ...
   请求耗时: ${requestDuration}ms
   响应头: ${JSON.stringify(responseHeaderLog, null, 2)}
   ...
   响应头: ${JSON.stringify(responseHeaderLog, null, 2)}`);
      ...
      if (isOllamaFormat) {
        ...
        if (data.usage) {
          ...
          const reasoningTokens = this.getReasoningTokenCount(data.usage);
        ...
        if (data.choices && data.choices[0]) {
          ...
          const reasoningContent = this.getReasoningContent(choice.message);
          console.log(`   响应内容:`);
          console.log(`     - Finish Reason: ${choice.finish_reason || 'N/A'}`);
          console.log(`     - 内容长度: ${choice.message?.content?.length || 0} 字符`);
          if (reasoningContent) {
            console.log(`     - Reasoning长度: ${reasoningContent.length} 字符`);
          }
        ...
        if (typeof content !== 'string' || content.trim().length === 0) {
          ...
          const reasoningTokens = this.getReasoningTokenCount(data.usage);
          const reasonHint = finishReason === 'length'
            ? '模型输出被 max_tokens 截断，且 token 主要消耗在 reasoning/思考内容中，未生成最终JSON。'
            : '模型未在 message.content 中返回可解析内容。';
          throw new Error(
            `AI返回内容为空：${reasonHint} finish_reason=${finishReason}, reasoning长度=${reasoningLength}字符, reasoning_tokens=${reasoningTokens}。请提高输出Token、关闭思考模式或换用非推理模型。`
          );
        }
      }
...
💡 提示：根据需求文档内容决定测试点数量，简单页面可能3-5个测试点，复杂流程可能需要10-20个测试点。`;
    ...
    try {
      ...
      // 🔥 仅当存在主流程或高风险门禁用例时，才自动补充 SMOKE
      const hasSmokeCase = testCases.some(tc => tc.caseType === 'SMOKE');
      if (!hasSmokeCase && testCases.length > 0) {
        const targetCase = testCases.find(tc => this.shouldPromoteCaseToSmoke(tc));
        if (targetCase) {
          console.warn('⚠️  警告：批次生成的关键门禁用例中没有冒烟用例，自动补充 SMOKE 类型');
      ...
      const finalTestCases = validTestCases;

      // 🆕 对测试用例进行排序：SMOKE（冒烟用例）优先，然后按优先级排序
      finalTestCases.sort((a, b) => {
        // 1. 首先按用例类型排序（SMOKE 最高优先级）
        const typeA = CASE_TYPE_SORT_ORDER[a.caseType || 'FULL'] || 99;
        const typeB = CASE_TYPE_SORT_ORDER[b.caseType || 'FULL'] || 99;
        if (typeA !== typeB) {
          return typeA - typeB;
        }
      ...
      console.log(`   📊 平均每个用例: ${finalTestCases.length > 0 ? (finalTotalTestPoints / finalTestCases.length).toFixed(1) : '0.0'} 个测试点`);
      console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`);
  ...
  private hasActionableChangeSummary(requirementDoc: string): boolean {
    const match = requirementDoc.match(/^##\s*变更摘要\s*[\r\n]+([\s\S]*?)(?=^##\s+|\s*$)/m);
    if (!match) return false;

    const summary = match[1].trim();
    if (!summary) return false;

    const negatedSummaryPatterns = [
      /不包含变更摘要/,
      /无(?:明确)?变更/,
      /没有(?:任何)?变更/,
      /而非针对特定版本的迭代说明/,
    ...
    if (negatedSummaryPatterns.some(pattern => pattern.test(summary))) {
      return false;
    }

    return /新增功能|修改功能|变更内容|本次迭代|本次变更|新增|修改|调整|删除|优化/.test(summary);
  }

  private extractRequirementRefs(text: string): string[] {
    return Array.from(new Set(text.match(/\bFR-[A-Z0-9](?:[A-Z0-9.-]*[A-Z0-9])?\b/g) || []))
      .filter(ref => ref !== 'FR-XXX');
  }

  private extractRequirementSections(text: string): string[] {
    const sections: string[] = [];
  ...
  private extractSectionKey(sectionRef: string): string {
    const trimmed = sectionRef.trim();
    const match = trimmed.match(/^(?:#{1,6}\s*)?((?:\d+(?:\.\d+)*|FR-[A-Z0-9.-]+))\b/i);
  ...
  private findSectionPosition(requirementDoc: string, sectionRef: string): number {
    const sectionKey = this.extractSectionKey(sectionRef);
    if (!sectionKey) return -1;

    const headingRegex = new RegExp(
      `^#{1,6}\\s+${this.escapeRegExp(sectionKey)}(?:\\s|[.：:]|$)`,
      'im'
    );
    const headingMatch = headingRegex.exec(requirementDoc);
    if (headingMatch) {
      return headingMatch.index;
    }

    const exactIndex = requirementDoc.indexOf(sectionRef);
    if (exactIndex >= 0) {
      return exactIndex;
    }

    if (/^FR-/.test(sectionKey)) {
      return requirementDoc.indexOf(sectionKey);
    }

    return -1;
  }

  private getScenarioDocumentOrder(scenario: TestScenario, requirementDoc: string, requirementRefs: string[]): number {
    const primarySectionPosition = (scenario.relatedSections || [])
      .map(ref => this.findSectionPosition(requirementDoc, ref))
      .find(index => index >= 0);

    if (primarySectionPosition !== undefined) {
      return primarySectionPosition;
    }

    const firstCoveredRefPosition = (scenario.coveredRequirementRefs || [])
      .map(ref => this.findSectionPosition(requirementDoc, ref))
      .find(index => index >= 0);

    if (firstCoveredRefPosition !== undefined) {
      return firstCoveredRefPosition;
    }

    const firstKnownRefIndex = (scenario.coveredRequirementRefs || [])
      .map(ref => requirementRefs.indexOf(ref))
      .filter(index => index >= 0)
      .sort((a, b) => a - b)[0];

    return firstKnownRefIndex === undefined ? Number.MAX_SAFE_INTEGER : requirementDoc.length + firstKnownRefIndex;
  }

  private extractSectionContent(requirementDoc: string, sectionRef: string): string {
    const sectionKey = this.extractSectionKey(sectionRef);
    const escapedSectionKey = this.escapeRegExp(sectionKey);

    const headingRegex = new RegExp(
      `^#{1,6}\\s+${escapedSectionKey}(?:[\\s.：:]+[^\\n]*)?\\n[\\s\\S]*?(?=^#{1,6}\\s+(?:\\d+(?:\\.\\d+)*|FR-[A-Z0-9.-]+)\\b|^\\s*-\\s+\\*\\*FR-|$)`,
      'im'
    );
    const headingMatch = requirementDoc.match(headingRegex);
    if (headingMatch) return headingMatch[0];

    const frRegex = new RegExp(
      `(^\\s*-\\s+\\*\\*${escapedSectionKey}\\b[\\s\\S]*?)(?=^\\s*-\\s+\\*\\*FR-|^#{1,6}\\s+|$)`,
      'im'
    );
    const frMatch = requirementDoc.match(frRegex);
    if (frMatch) return frMatch[1];

    return '';
  }

  private logScenarioCoverage(requirementRefs: string[], scenarios: TestScenario[]): void {
    if (requirementRefs.length === 0) return;

    const explicitRefs = scenarios.flatMap(s => s.coveredRequirementRefs || []);
    const fallbackText = scenarios
      .flatMap(s => s.relatedSections || [])
      .join('\n');
    const coveredRefs = new Set([
      ...this.extractRequirementRefs(explicitRefs.join('\n')),
      ...this.extractRequirementRefs(fallbackText)
    ]);
    const uncoveredRefs = requirementRefs.filter(ref => !coveredRefs.has(ref));
    const coverageRate = Math.round((coveredRefs.size / requirementRefs.length) * 100);

    console.log(`📊 [测试场景拆分] FR覆盖率: ${coveredRefs.size}/${requirementRefs.length} (${coverageRate}%)`);
    if (uncoveredRefs.length > 0) {
      console.warn(`⚠️ [测试场景拆分] 未直接关联的FR示例: ${uncoveredRefs.slice(0, 30).join(', ')}${uncoveredRefs.length > 30 ? '...' : ''}`);
    }
  }

  private inferTestPointType(testPoint: TestPoint): TestPoint['testPointType'] {
    const text = `${testPoint.testPoint}\n${testPoint.description || ''}\n${testPoint.steps || ''}\n${testPoint.expectedResult || ''}`;
    if (/边界|最小|最大|超过|低于|阈值|为空|空值|长度|位数|过期|失效/.test(text)) return 'boundary';
    if (/权限|越权|角色|隔离|菜单/.test(text)) return 'permission';
    if (/锁定|解锁|超时|有效期|状态|会话/.test(text)) return 'state';
    if (/安全|验证码|密码|双因子|证书|HTTPS/.test(text)) return 'security';
    if (/错误|失败|异常|非法|无效|不一致|不符合/.test(text)) return 'abnormal';
    if (this.isMainFlowTestPoint(testPoint.testPoint)) return 'main';
    return 'abnormal';
  }

  private inferRequirementRefsForTestPoint(testPoint: TestPoint, candidateRefs: string[]): string[] {
    const text = `${testPoint.testPoint}\n${testPoint.description || ''}\n${testPoint.coverageAreas || ''}\n${testPoint.steps || ''}\n${testPoint.expectedResult || ''}`;
    const inferred = new Set<string>();
    const hasRef = (ref: string) => candidateRefs.includes(ref);
    const add = (ref: string) => {
      if (hasRef(ref)) inferred.add(ref);
    };

    if (/三权|权限隔离|越权|角色|平台/.test(text)) add('FR-001');
    if (/登录|初始密码|强制改密|管理口|HTTPS/.test(text)) add('FR-009');
    if (/信任站点|火狐|Firefox|证书例外/.test(text)) add('FR-B-001');
    if (/用户名|密码|验证码|账号锁定|登录失败|登录页面/.test(text)) add('FR-C-001');
    if (/磁盘|CPU|FTP|NAS|登录前|系统异常告警/.test(text)) add('FR-C-002');
    if (/个人修改密码|旧密码|新密码|确认新密码|密码强度/.test(text)) add('FR-E-002');
    if (/用户管理|子用户|新增用户|编辑用户|删除用户|重置密码|锁定用户|权限分配/.test(text)) add('FR-E-003');

    return Array.from(inferred);
  }

  private estimateTestCasesForTestPoint(testPoint: TestPoint): number {
    const text = [
      testPoint.testPoint,
    ...
    const aiEstimate = Number(testPoint.estimatedTestCases || 0);
    const type = testPoint.testPointType || this.inferTestPointType(testPoint);

    if (/密码复杂度|复杂度不满足|不满足复杂度|不符合复杂度|密码.*不符合|密码策略/.test(text)) {
      return Math.max(8, Math.min(10, Math.round(aiEstimate) || 8));
    }

    if (/登录失败|账号锁定|失败锁定|锁定阈值|禁止登录时间|锁定时长/.test(text)) {
      return Math.max(5, Math.min(6, Math.round(aiEstimate) || 5));
    }

    if (/双因子|短信验证码|二次认证|2fa|验证码.*6位/.test(text)) {
      return Math.max(6, Math.min(8, Math.round(aiEstimate) || 6));
    }

    if (/图形验证码|验证码规则|验证码.*有效期|验证码.*刷新|验证码.*过期/.test(text)) {
      return Math.max(5, Math.min(6, Math.round(aiEstimate) || 5));
    }

    if (/黑白名单|黑名单|白名单|ip地址段|ip范围|名单优先/.test(text)) {
      return Math.max(4, Math.min(6, Math.round(aiEstimate) || 4));
    }

    if (/告警方式|邮件|短信|syslog|snmp|企业微信|钉钉|通知方式/.test(text)) {
      return Math.max(4, Math.min(6, Math.round(aiEstimate) || 4));
    }

    if (/导入.*导出|导出.*导入|导入|导出|excel|word|pdf|zip|文件格式|文件大小|上传|下载/.test(text)) {
      return Math.max(3, Math.min(6, Math.round(aiEstimate) || 3));
    }

    if (/高级检索|组合查询|多条件|检索模板|显示列|更多筛选|审计记录.*导出|查询条件/.test(text)) {
      return Math.max(5, Math.min(8, Math.round(aiEstimate) || 5));
    }

    if (/新增|添加|编辑|修改|删除|复制|查询|启用|禁用|批量|状态切换|crud/.test(text)) {
      return Math.max(4, Math.min(7, Math.round(aiEstimate) || 4));
    }

    if (/备份|恢复|清理|外置存储|自动任务|手动任务|180天|磁盘使用率|存储方式/.test(text)) {
      return Math.max(6, Math.min(9, Math.round(aiEstimate) || 6));
    }

    if (/审计策略|规则组|规则管理|普通规则|组合规则|匹配优先级|阻断|引用关系/.test(text)) {
      return Math.max(5, Math.min(8, Math.round(aiEstimate) || 5));
    }

    if (/保护对象|自动发现|端口扫描|流量识别|漏洞扫描|状态监控|扩展信息/.test(text)) {
      return Math.max(5, Math.min(8, Math.round(aiEstimate) || 5));
    }

    if (/报表|图表|趋势|排行|top5|自定义报表|自动报表|合规报表|邮件发送/.test(text)) {
      return Math.max(4, Math.min(7, Math.round(aiEstimate) || 4));
    }

    if (/敏感发现|敏感类型|数据字典|敏感分析|定时|周期执行|连接测试/.test(text)) {
      return Math.max(5, Math.min(8, Math.round(aiEstimate) || 5));
    }

    if (/部署方式|网口|管理口|静态路由|多级部署|中心节点|子节点|agent|镜像/.test(text)) {
      return Math.max(5, Math.min(8, Math.round(aiEstimate) || 5));
    }

    if (/许可证|授权证书|机器码|过期|引擎状态|上传证书|激活/.test(text)) {
      return Math.max(4, Math.min(6, Math.round(aiEstimate) || 4));
    }

    if (/升级|回退|重启|关机|恢复出厂|抓包|日志收集|系统维护/.test(text)) {
      return Math.max(4, Math.min(7, Math.round(aiEstimate) || 4));
    }

    if (/长度|字符|位数|最小|最大|低于|超过|边界|阈值|上限|下限|至少|最多/.test(text)) {
      return Math.max(4, Math.min(6, Math.round(aiEstimate) || 4));
    }

    if (/sysadmin.*secadmin.*auditadmin|三个默认管理员|三类管理员|三种角色|三权分立.*角色/.test(text)) {
      return Math.max(3, Math.min(4, Math.round(aiEstimate) || 3));
    }

    if (/多种|不同角色|多个角色|组合|分别|批量|多条件|多数据/.test(text)) {
      return Math.max(2, Math.min(3, Math.round(aiEstimate) || 2));
    }

    if (type === 'permission' || type === 'security' || type === 'state') {
      return Math.max(1, Math.min(2, Math.round(aiEstimate) || 1));
    }
    ...
    if (type === 'main' || type === 'abnormal' || type === 'regression') {
      return Math.max(1, Math.min(2, Math.round(aiEstimate) || 1));
    }

    return Math.max(1, Math.min(3, Math.round(aiEstimate) || 1));
  }

  private buildAdditionalRequirementContextForTestPoint(testPoint: TestPoint, requirementDoc: string): string {
    const text = `${testPoint.testPoint}\n${testPoint.description || ''}\n${testPoint.steps || ''}\n${testPoint.expectedResult || ''}`;
    const refs = new Set<string>();

    if (/密码复杂度|复杂度不满足|不满足复杂度|不符合复杂度|密码.*不符合|密码策略|强制改密/.test(text)) {
      refs.add('FR-074');
      refs.add('FR-C-001');
    }
    if (/登录失败|账号锁定|锁定阈值/.test(text)) {
      refs.add('FR-075');
      refs.add('FR-C-001');
    }
    if (/验证码|登录表单|用户名|密码为空|密码长度/.test(text)) {
      refs.add('FR-C-001');
    }

    return Array.from(refs)
      .map(ref => this.extractSectionContent(requirementDoc, ref))
      .filter(Boolean)
      .join('\n\n');
  }

  private normalizeTestPointMetadata(testPoints: TestPoint[], requirementContext: string): void {
    const knownRefs = new Set(this.extractRequirementRefs(requirementContext));
    const fallbackRefs = Array.from(knownRefs);

    testPoints.forEach(tp => {
      const refs = this.extractRequirementRefs([
        ...(tp.coveredRequirementRefs || []),
        tp.testPoint,
        tp.description || '',
        tp.coverageAreas || '',
        tp.steps || '',
      ...
      const inferredRefs = refs.length > 0
        ? refs
        : this.inferRequirementRefsForTestPoint(tp, fallbackRefs);
      tp.coveredRequirementRefs = Array.from(new Set(
        inferredRefs.length > 0 ? inferredRefs : (fallbackRefs.length <= 3 ? fallbackRefs : [])
      ));
      tp.source = tp.source || 'requirement';
      tp.sourceLabel = tp.sourceLabel || (tp.source === 'supplemental' ? '规则补充' : '需求明确');
      tp.testPointType = tp.testPointType || this.inferTestPointType(tp);
      tp.estimatedTestCases = this.estimateTestCasesForTestPoint(tp);
    });
  }

  private isHighRiskLoginScenario(name: string, description: string, requirementContext: string): boolean {
    const text = `${name}\n${description}\n${requirementContext}`;
    return /登录|认证|强制改密|三权分立|默认管理员|验证码|登录失败|账号锁定|密码复杂度/.test(text)
      && /high|高风险|核心|权限|三权|管理员|安全/.test(text);
  }

  private buildLoginSecuritySupplementalTestPoints(existing: TestPoint[], requirementContext: string): TestPoint[] {
    const text = existing.map(tp => `${tp.testPoint}\n${tp.description || ''}`).join('\n');
    const refs = this.extractRequirementRefs(requirementContext);
    const pickRefs = (...candidates: string[]) => candidates.filter(ref => refs.includes(ref));
    const fallbackRefs = refs.slice(0, 3);
    const reqRefs = (...candidates: string[]) => {
      const matched = pickRefs(...candidates);
      return matched.length > 0 ? matched : fallbackRefs;
    };

    const addIfMissing = (
      pattern: RegExp,
      point: Omit<TestPoint, 'testScenario'>
    ): TestPoint[] => pattern.test(text) ? [] : [{
      ...point,
      source: point.source || 'supplemental',
      sourceLabel: point.sourceLabel || '规则补充',
      sourceReason: point.sourceReason || '登录/认证属于高风险入口，按安全测试基线补充验证点。'
    } as TestPoint];

    return [
      ...addIfMissing(/用户名为空|用户名.*空值|空用户名/, {
        testPoint: '登录时用户名为空，提示必填并阻止提交',
        testPointType: 'boundary',
        coveredRequirementRefs: reqRefs('FR-C-001'),
        description: '验证登录表单用户名必填校验，用户名为空时不能继续登录。',
        coverageAreas: '登录表单必填校验',
        estimatedTestCases: 1,
        steps: '1. 打开登录页面\n2. 用户名输入框保持为空\n3. 输入有效密码和验证码\n4. 点击登录',
        expectedResult: '系统提示用户名不能为空或必填，登录请求不通过，不进入强制改密或系统首页。',
        riskLevel: 'medium'
      }),
      ...addIfMissing(/密码为空|密码.*空值|空密码/, {
        testPoint: '登录时密码为空，提示必填并阻止提交',
        testPointType: 'boundary',
        coveredRequirementRefs: reqRefs('FR-C-001'),
        description: '验证登录表单密码必填校验，密码为空时不能继续登录。',
        coverageAreas: '登录表单必填校验',
        estimatedTestCases: 1,
        steps: '1. 打开登录页面\n2. 输入有效用户名\n3. 密码输入框保持为空\n4. 输入有效验证码\n5. 点击登录',
        expectedResult: '系统提示密码不能为空或必填，登录请求不通过，不进入强制改密或系统首页。',
        riskLevel: 'medium'
      }),
      ...addIfMissing(/验证码.*为空|验证码.*空值|空验证码/, {
        testPoint: '登录时验证码为空，提示必填并阻止提交',
        testPointType: 'boundary',
        coveredRequirementRefs: reqRefs('FR-C-001'),
        description: '验证登录验证码必填校验，验证码为空时不能提交登录。',
        coverageAreas: '验证码必填校验',
        estimatedTestCases: 1,
        steps: '1. 打开登录页面\n2. 输入有效用户名和密码\n3. 验证码输入框保持为空\n4. 点击登录',
        expectedResult: '系统提示验证码不能为空或必填，登录失败，页面停留在登录页。',
        riskLevel: 'medium'
      }),
      ...addIfMissing(/验证码.*错误|错误验证码/, {
        testPoint: '登录时验证码错误，提示校验失败并阻止登录',
        testPointType: 'abnormal',
        coveredRequirementRefs: reqRefs('FR-C-001'),
        description: '验证输入错误验证码时系统拒绝登录并给出明确提示。',
        coverageAreas: '验证码错误处理',
        estimatedTestCases: 1,
        steps: '1. 打开登录页面\n2. 输入有效用户名和密码\n3. 输入错误验证码\n4. 点击登录',
        expectedResult: '系统提示验证码错误或验证码校验失败，登录失败，页面停留在登录页。',
        riskLevel: 'medium'
      }),
      ...addIfMissing(/验证码.*过期|验证码.*失效/, {
        testPoint: '登录验证码超过有效期后提交，提示验证码失效',
        testPointType: 'state',
        coveredRequirementRefs: reqRefs('FR-C-001'),
        description: '验证验证码超过需求规定有效期后不能继续用于登录。',
        coverageAreas: '验证码有效期校验',
        estimatedTestCases: 1,
        steps: '1. 打开登录页面并获取验证码\n2. 等待超过需求规定的验证码有效期\n3. 输入有效用户名、密码和已过期验证码\n4. 点击登录',
        expectedResult: '系统提示验证码过期或失效，登录失败，并要求刷新验证码。',
        riskLevel: 'medium'
      }),
      ...addIfMissing(/登录失败.*锁定|锁定阈值|失败次数/, {
        testPoint: '连续登录失败达到锁定阈值后账号被锁定',
        testPointType: 'state',
        coveredRequirementRefs: reqRefs('FR-C-001', 'FR-075'),
        description: '验证同一账号连续登录失败达到需求规定阈值后触发账号锁定。',
        coverageAreas: '登录失败锁定策略',
        estimatedTestCases: 1,
        steps: '1. 打开登录页面\n2. 使用同一管理员账号连续输入错误密码\n3. 失败次数达到需求规定锁定阈值\n4. 再次使用正确密码尝试登录',
        expectedResult: '达到锁定阈值后账号被锁定，正确密码也不能登录，并提示账号锁定或稍后再试。',
        riskLevel: 'high'
      }),
      ...addIfMissing(/确认密码.*不一致|两次密码.*不一致/, {
        testPoint: '强制改密时两次新密码不一致，提示错误并阻止修改',
        testPointType: 'abnormal',
        coveredRequirementRefs: reqRefs('FR-C-001', 'FR-074'),
        description: '验证强制改密页面的新密码和确认密码一致性校验。',
        coverageAreas: '强制改密一致性校验',
        estimatedTestCases: 1,
        steps: '1. 使用默认管理员账号登录并进入强制改密页面\n2. 输入符合复杂度的新密码\n3. 确认密码输入不同值\n4. 点击确认修改',
        expectedResult: '系统提示两次密码输入不一致，修改失败，页面停留在强制改密页。',
        riskLevel: 'medium'
      }),
      ...addIfMissing(/新密码.*旧密码相同|与旧密码相同|新旧密码一致/, {
        testPoint: '强制改密时新密码与旧密码相同，提示错误并阻止修改',
        testPointType: 'security',
        coveredRequirementRefs: reqRefs('FR-C-001', 'FR-074'),
        description: '验证强制改密时不能继续使用初始密码或旧密码作为新密码。',
        coverageAreas: '强制改密安全策略',
        estimatedTestCases: 1,
        steps: '1. 使用默认管理员账号登录并进入强制改密页面\n2. 新密码输入与当前默认密码相同的值\n3. 确认密码输入相同值\n4. 点击确认修改',
        expectedResult: '系统提示新密码不能与旧密码相同或不符合安全策略，修改失败。',
        riskLevel: 'medium'
      }),
      ...addIfMissing(/加密传输|HTTPS.*登录|密码.*传输|明文传输/, {
        testPoint: '登录密码加密传输安全验证',
        testPointType: 'security',
        coveredRequirementRefs: reqRefs('FR-009', 'FR-C-001'),
        description: '验证登录过程通过 HTTPS 安全通道提交凭据，密码不以明文形式在网络传输中暴露。',
        coverageAreas: '登录传输安全、密码加密传输',
        estimatedTestCases: 1,
        steps: '1. 打开登录页面并保持浏览器开发者工具网络面板开启\n2. 输入有效用户名、密码和验证码\n3. 点击登录并查看登录请求协议与请求载荷\n4. 检查网络传输内容',
        expectedResult: '登录请求使用 HTTPS 或需求规定的安全通道，网络请求中不直接暴露明文密码，传输过程符合安全要求。',
        riskLevel: 'high'
      }),
      ...addIfMissing(/暴力破解|连续.*错误密码|登录失败.*防护|破解防护/, {
        testPoint: '连续错误密码登录触发暴力破解防护',
        testPointType: 'security',
        coveredRequirementRefs: reqRefs('FR-C-001', 'FR-075'),
        description: '验证同一账号连续错误密码登录时，系统能够触发失败限制、锁定或验证码增强等防护策略。',
        coverageAreas: '暴力破解防护、登录失败限制',
        estimatedTestCases: 1,
        steps: '1. 打开登录页面\n2. 使用同一管理员账号连续输入错误密码并提交登录\n3. 达到需求规定失败次数或防护条件\n4. 再次尝试登录',
        expectedResult: '系统按需求触发登录失败防护策略，如账号锁定、限制登录、增强验证码或明确安全提示，不能被无限次尝试破解。',
        riskLevel: 'high'
      }),
      ...addIfMissing(/SQL注入|sql.*注入|注入防护/, {
        testPoint: '登录表单SQL注入防护安全验证',
        testPointType: 'security',
        coveredRequirementRefs: reqRefs('FR-C-001'),
        description: '验证用户名、密码等登录输入框对 SQL 注入载荷具备拦截或安全处理能力。',
        coverageAreas: 'SQL注入防护、登录输入安全',
        estimatedTestCases: 1,
        steps: '1. 打开登录页面\n2. 在用户名或密码输入框输入典型 SQL 注入载荷\n3. 输入验证码并提交登录\n4. 观察系统响应与后台状态',
        expectedResult: '系统拒绝登录并给出安全提示或通用失败提示，不执行异常查询，不绕过认证，不暴露数据库错误信息。',
        riskLevel: 'high'
      }),
      ...addIfMissing(/XSS|跨站脚本|脚本防护/, {
        testPoint: '登录表单XSS跨站脚本防护安全验证',
        testPointType: 'security',
        coveredRequirementRefs: reqRefs('FR-C-001'),
        description: '验证登录输入框对脚本载荷进行安全处理，不在页面提示或回显中执行脚本。',
        coverageAreas: 'XSS防护、登录输入安全',
        estimatedTestCases: 1,
        steps: '1. 打开登录页面\n2. 在用户名或密码输入框输入典型 XSS 脚本载荷\n3. 输入验证码并提交登录\n4. 观察页面提示、回显和脚本执行情况',
        expectedResult: '系统不执行脚本，不产生跨站脚本弹窗或恶意跳转，输入内容被拦截、转义或按普通文本安全处理。',
        riskLevel: 'medium'
      }),
      ...addIfMissing(/会话超时|session.*timeout|超时失效|登录会话/, {
        testPoint: '登录会话超时后访问受保护页面被拦截',
        testPointType: 'security',
        coveredRequirementRefs: reqRefs('FR-C-001'),
        description: '验证用户登录后会话超过需求规定有效期时，继续访问受保护页面会被要求重新登录。',
        coverageAreas: '会话超时、安全认证状态',
        estimatedTestCases: 1,
        steps: '1. 使用有效管理员账号登录系统\n2. 进入任一受保护页面\n3. 等待超过需求规定的会话有效期或模拟会话失效\n4. 刷新页面或继续访问受保护接口',
        expectedResult: '系统拦截已失效会话，跳转登录页或提示重新登录，不允许继续访问受保护资源。',
        riskLevel: 'medium'
      })
    ];
  }
  ...
  async analyzeTestScenarios(requirementDoc: string): Promise<TestScenario[]> {
    ...
    // 🆕 检测是否为修改页面模式（必须存在真实可执行的变更摘要）
    const isModifyMode = this.hasActionableChangeSummary(requirementDoc);
    const requirementRefs = this.extractRequirementRefs(requirementDoc);
    const sectionHeadings = this.extractRequirementSections(requirementDoc);
    const isLargeRequirementDoc = requirementRefs.length >= 40 || sectionHeadings.length >= 60 || requirementDoc.length >= 40000;
    const recommendedScenarioMin = isLargeRequirementDoc
      ? Math.min(30, Math.max(15, Math.ceil(requirementRefs.length / 8)))
      : 3;
    const recommendedScenarioMax = isLargeRequirementDoc
      ? Math.min(60, Math.max(recommendedScenarioMin + 8, Math.ceil(requirementRefs.length / 3)))
      : 6;
    const scenarioCountGuidance = isLargeRequirementDoc
      ? `- 当前是大型需求文档：识别到 ${requirementRefs.length} 个唯一需求编号、${sectionHeadings.length} 个标题章节。请按功能域/核心FR拆分，建议输出 ${recommendedScenarioMin}-${recommendedScenarioMax} 个测试场景；禁止压缩成 3-6 个大场景。`
      : '- 场景数量控制在3-6个，太多会导致规划过于分散';
    const requirementRefsBlock = requirementRefs.length > 0
      ? `\n## 📌 已识别需求编号清单（coveredRequirementRefs 只能从这里或原文中选择）\n${requirementRefs.join(', ')}\n`
      : '';
  ...
  "scenarios": [
    {
      ...
      "coveredRequirementRefs": ["FR-001", "FR-C-001"],
...
**重要：coveredRequirementRefs 格式说明**
- 每个测试场景必须输出 coveredRequirementRefs 数组，列出该场景直接覆盖的需求编号
- 需求编号必须来自需求文档原文，例如 "FR-001"、"FR-A-001"、"FR-5.10.7-01"
- 如果同一场景覆盖多个FR，必须全部列出；不要只写章节号
- 禁止编造需求编号；无法确认时宁可少列，但P0/P1核心需求必须尽量完整映射
...
6. **每个场景必须有 coveredRequirementRefs，列出直接覆盖的FR编号；这是覆盖率统计依据！**
...
## 重要提示
${scenarioCountGuidance}
- 场景必须覆盖P0/P1核心需求；大型文档应按登录权限、部署许可、策略规则、保护对象、审计检索、风险报表、告警接口、数据维护、安全配置、敏感/阻断/态势等功能域拆分，避免把多个独立FR合并成一个笼统场景
- 优先级判断：功能性 > 权限 > 布局文案
- 每个场景要有清晰的测试边界，避免重叠
...
- [ ] 每个场景的 coveredRequirementRefs 至少有 1 个需求编号，且编号来自需求文档？
- [ ] P0/P1 核心需求编号是否已分配到某个场景？
...
## 🔥 排序要求（最高优先级！）
⚠️ **输出的测试场景数组必须严格按照功能模块顺序排序，同一模块内再按照优先级排序！**

**强制排序规则：**
1. **主排序：按照需求文档中的功能模块/章节出现顺序排序**
   - 先出现的一级/二级功能模块，其测试场景排在前面
   - 同一功能模块下的多个场景保持相邻，不要被其他模块打散

2. **次排序：同一功能模块内按优先级排序：high → medium → low**
   - 同一模块内 high 优先级场景排在 medium 之前
   - 同一模块内 medium 优先级场景排在 low 之前

3. **第一个场景应该是文档顺序中最靠前的核心主流程场景，优先级通常为 "high"**
   - 核心业务功能、主要操作流程
   - 例如：登录场景、订单创建场景、数据查询场景

4. **输出前检查清单：**
   - [ ] 场景是否基本遵循需求文档章节/功能模块顺序？
   - [ ] 同一模块下的场景是否相邻？
   - [ ] 同一模块内是否按 high → medium → low 排序？

**正确示例：**
[{module:"1 登录", priority:"high"}, {module:"1 登录", priority:"medium"}, {module:"2 用户", priority:"high"}, {module:"2 用户", priority:"low"}] ✅
**错误示例：**
[{module:"1 登录", priority:"high"}, {module:"2 用户", priority:"high"}, {module:"1 登录", priority:"medium"}] ❌
...
请识别页面类型，并根据上述原则拆分测试场景。**确保主流程场景排在第一位！** 直接输出JSON格式，不要其他说明文字。`;
    ...
    try {
      ...
      const scenarioMaxTokens = this.resolveScenarioAnalysisMaxTokens(currentConfig, isLargeRequirementDoc);
      if (scenarioMaxTokens > (currentConfig.maxTokens || 0)) {
        console.log(`   场景拆分输出Token上限: 全局=${currentConfig.maxTokens || '未配置'} → 本次=${scenarioMaxTokens}`);
      }
      const aiResponse = await this.callAI(systemPrompt, userPrompt, scenarioMaxTokens);

      // 解析JSON
      const parsed = this.parseAIJsonResponse(aiResponse, '测试场景拆分');
      const scenarios: TestScenario[] = parsed.scenarios || [];
      const knownRequirementRefs = new Set(requirementRefs);
      scenarios.forEach(s => {
        const refsFromRelatedSectionContent = (s.relatedSections || [])
          .flatMap(sectionId => this.extractRequirementRefs(this.extractSectionContent(requirementDoc, sectionId)));
        const refsFromScenario = this.extractRequirementRefs([
          ...(s.coveredRequirementRefs || []),
          ...(s.relatedSections || []),
          ...refsFromRelatedSectionContent,
          s.name,
          s.description
        ].join('\n'));
        s.coveredRequirementRefs = Array.from(new Set(refsFromScenario.filter(ref => knownRequirementRefs.has(ref))));
      });
      this.logScenarioCoverage(requirementRefs, scenarios);
      if (isLargeRequirementDoc && scenarios.length < recommendedScenarioMin) {
        console.warn(`⚠️ [测试场景拆分] 大型需求文档仅生成 ${scenarios.length} 个场景，低于建议下限 ${recommendedScenarioMin}。请检查模型是否仍在过度合并功能域。`);
      }

      // 🆕 确保测试场景按需求文档功能模块顺序排序，同一模块内再按优先级排序
      scenarios.sort((a, b) => {
        const priorityOrder = { high: 1, medium: 2, low: 3 };
        const documentOrderA = this.getScenarioDocumentOrder(a, requirementDoc, requirementRefs);
        const documentOrderB = this.getScenarioDocumentOrder(b, requirementDoc, requirementRefs);
        if (documentOrderA !== documentOrderB) {
      ...
      console.log(`✅ 成功拆分 ${scenarios.length} 个测试场景（已按功能模块顺序排序，同一模块内按优先级排序）:`);
      scenarios.forEach((s, i) => {
...
╚═══════════════════════════════════════════════════════════════╝\n`);
    ...
    // 提取相关章节内容 - 支持1-6级标题、FR标题以及"编号 标题"格式的引用
    const sectionContents = relatedSections
      .map(sectionId => this.extractSectionContent(requirementDoc, sectionId))
      .filter(Boolean)
      .join('\n\n');
...
1. **正常流程**：验证标准操作路径
2. **异常流程**：验证错误处理和提示
3. **边界条件**：验证特殊值、空值、超长输入、**长度/数值范围临界值**（见下方专项规则）
4. **权限控制**：验证不同角色的权限限制

## 🔥 长度/数值范围边界（强制，从需求文档自动识别）

**不要套用固定数字！** 必须先阅读下方「相关需求内容」，从中识别每个字段/规则的长度或数值范围（Min～Max），再按识别结果生成测试点。

**常见表述（均需从文档原文提取 Min、Max，勿臆造）：**
- 「X-Y字符」「X至Y字」「长度X到Y」「最少X最多Y字符」
- 原型/表格中的「0/100」「X/Y」长度列、字段说明里的字数限制
- 「不少于X字」「不超过Y字」「至少X位」「至多Y位」

**对每个识别出的范围 [Min, Max]（及对应字段名），生成边界测试点（Min>0 时四类都要；Min=0 时重点覆盖等于最大、超过最大）：**

| 边界类型 | 命名模式（将 Min/Max 替换为文档中的实际数字） |
|---------|-----------------------------------------------|
| 低于最小值 | {字段}输入 (Min-1) 字符（低于最小值 Min） |
| 等于最小值 | {字段}输入恰好 Min 字符（等于最小边界） |
| 等于最大值 | {字段}输入恰好 Max 字符（等于最大边界） |
| 超过最大值 | {字段}输入 (Max+1) 字符（超过最大值 Max） |

**补充规则：**
- 多个字段、多个范围 → 每个范围独立生成上述测试点
- 测试点名称、steps、expectedResult 中的数字必须与需求文档一致
- 主流程可用范围内合法中间值（非固定数字，由 Min/Max 推算）
- 空值/必填与长度边界分开设计
...
3. **后续：边界条件和次要异常**
   - 空值/必填：如 "用户名为空"、"密码为空"
   - **长度/数值边界**：按需求文档中的 Min/Max 生成，如 "输入(Min-1)字符（低于最小值Min）"、"恰好Min字符"、"恰好Max字符"、"(Max+1)字符（超过最大值Max）"
   - 风险等级：medium 或 low

**识别主流程的关键词：**
- ✅ 正确、正常、有效、成功、合法、标准、凭据正确、密码正确、数据有效、范围内有效值
- ❌ 错误、无效、失败、异常、非法、为空、超长、凭据错误、密码错误、低于最小、超过最大
- ⚠️ 主流程测试点名称不要用「边界」二字，但**必须另外生成**长度/数值边界类测试点
...
- [ ] 是否已从需求文档识别所有长度/数值范围，并为每个范围生成对应边界测试点（数字与文档一致，非模板照搬）？
- [ ] 每个测试点是否都有 testPointType？
- [ ] 每个测试点是否都有 coveredRequirementRefs，且编号来自需求文档？
- [ ] 每个测试点是否都有 source/sourceLabel？需求原文直接覆盖为 requirement/需求明确；规则补强为 supplemental/规则补充
  ...
  "testPoints": [
    {
      ...
      "coveredRequirementRefs": ["FR-C-001", "FR-009"],
      "source": "requirement",
      "sourceLabel": "需求明确",
      "sourceReason": "需求文档明确描述该校验点",
...
- **testPointType**: 测试点类型，必须从以下枚举选择：main（主流程）、abnormal（异常流程）、boundary（边界值）、permission（权限隔离）、state（状态/时效）、security（安全约束）、regression（回归检查）
- **coveredRequirementRefs**: 关联需求编号数组，必须来自需求文档原文，例如 ["FR-C-001", "FR-009"]；禁止编造编号；边界测试点也必须绑定触发该边界规则的FR编号
- **source/sourceLabel/sourceReason**: 测试点来源。需求原文明确要求的用 source="requirement"、sourceLabel="需求明确"；基于安全/质量基线补充的用 source="supplemental"、sourceLabel="规则补充"，并在 sourceReason 说明补充依据
...
` : ''}`;
    ...
    const requirementContext = `${scenarioRequirementContext}\n${scenarioDescription}`;
    const detectedRanges = this.extractRangeConstraintsFromText(requirementContext);
    const isHighRiskLoginScenario = this.isHighRiskLoginScenario(scenarioName, scenarioDescription, requirementContext);
    const detectedRangesBlock =
      detectedRanges.length > 0
        ? `\n## 📏 系统已从需求文档自动识别的长度/数值范围（必须据此生成边界测试点，勿使用其他数字）\n${detectedRanges
            .map(c => {
              const field = c.fieldName ? `字段「${c.fieldName}」` : '未指名字段';
              const below = c.min > 0 ? `、${c.min - 1}（低于最小 ${c.min}）` : '';
              return `- ${field}：最小 ${c.min}，最大 ${c.max} → 需覆盖：${c.min}（最小边界）${below}、${c.max}（最大边界）、${c.max + 1}（超过最大 ${c.max}）`;
            })
            .join('\n')}\n`
        : '\n## 📏 长度/数值范围\n若下方需求中出现任何长度、字数、位数限制，请自行从原文提取 Min/Max 并生成对应边界测试点。\n';
...
- 避免为未变更的原有功能生成测试点
` : ''}
${isHighRiskLoginScenario ? `
## 🚨 高风险登录/认证场景专项要求
- 当前场景涉及登录、认证、强制改密、三权分立或账号安全，目标测试点数量应为 **12-16 个**。
- 必须覆盖：正常登录/首次强制改密、三角色权限隔离、错误密码、用户名为空、密码为空、验证码为空、验证码错误、验证码过期/失效、登录失败锁定阈值、强制改密复杂度不满足、两次新密码不一致、新密码与旧密码相同、未完成改密状态保持、登录页异常告警。
- 如果需求文档给出用户名/密码/验证码长度、失败次数、有效期等明确阈值，必须按阈值生成边界点；如果没有明确阈值，只生成语义校验点，不得编造数字。
` : ''}
请根据需求内容，为该场景生成详细的测试点。长度/数值边界测试点中的具体数字必须来自上方需求或「已识别范围」列表，禁止编造与文档不符的阈值。
...
直接输出JSON格式，不要其他说明文字。`;
    ...
    try {
      ...
      let testPoints: TestPoint[] = parsed.testPoints || [];
      ...
      // 📏 根据需求文档自动识别的长度范围，补充 AI 遗漏的边界测试点
      const beforeCount = testPoints.length;
      testPoints = this.supplementBoundaryTestPoints(testPoints, requirementContext, scenarioName);
      if (testPoints.length > beforeCount) {
        console.log(`   📏 已根据长度范围规则补充 ${testPoints.length - beforeCount} 个边界测试点`);
      }

      if (isHighRiskLoginScenario) {
        const beforeLoginSupplementCount = testPoints.length;
        testPoints = [
          ...testPoints,
          ...this.buildLoginSecuritySupplementalTestPoints(testPoints, requirementContext)
        ];
        if (testPoints.length > beforeLoginSupplementCount) {
          console.log(`   🔐 高风险登录场景已补充 ${testPoints.length - beforeLoginSupplementCount} 个登录/认证专项测试点`);
        }
      }

      this.normalizeTestPointMetadata(testPoints, requirementContext);

      // 🆕 智能排序：主流程优先 + 风险等级排序
      testPoints.sort((a, b) => {
        // 1. 识别主流程测试点（正常流程优先）
        const isMainFlowA = a.testPointType === 'main' || this.isMainFlowTestPoint(a.testPoint);
        const isMainFlowB = b.testPointType === 'main' || this.isMainFlowTestPoint(b.testPoint);
  ...
  async generateTestCaseForTestPoint(
    ...
    // 🆕 检测是否为修改页面模式
    const isModifyMode = this.hasActionableChangeSummary(requirementDoc);
    const baseRequirementContext = relatedSections
      .map(sectionId => this.extractSectionContent(requirementDoc, sectionId))
      .filter(Boolean)
      .join('\n\n') || requirementDoc.substring(0, 4000);
    const additionalRequirementContext = this.buildAdditionalRequirementContextForTestPoint(testPoint, requirementDoc);
    const testCaseRequirementContext = [
      baseRequirementContext,
...
- 每个测试用例包含：用例名称、测试步骤、预期结果、前置条件、测试数据等
    ...
    **何时生成多个测试用例？**
    - ✅ **推荐**：测试点涉及多种测试数据 → 按系统给出的推荐数量生成
    - ✅ **推荐**：测试点涉及多种场景 → 按角色/场景分别生成
    - ✅ **推荐**：测试点需要不同的前置条件 → 按前置条件分别生成
    - ⚠️ **默认**：简单单点测试 → 生成1个用例
    - ✅ **必须**：复杂测试点（多种组合、边界、策略等级）→ 生成多条用例完整覆盖
...
- 正常流程用例应该只在"正常流程"测试点中生成
    ...
    **预估测试用例数量的原则：**
    - 分析测试点的复杂度：步骤数量、涉及的数据类型、场景数量、边界数量和策略等级
    - 简单测试点（单一场景、单一数据）：**生成1个用例**
    - 中等测试点（多种数据组合）：**生成2-3个用例**
    - 复杂策略/边界测试点：按后续推荐数量生成，不能压缩覆盖点
...
⚠️ **SMOKE（冒烟）表示关键门禁用例，不只限于正向主流程！**

- **SMOKE（冒烟用例）**【优先级必须为 high】：用于快速判断版本是否具备最小可验收能力的关键门禁用例
  * 示例：用户登录的正常流程、订单创建的基本流程、数据查询的主要功能
  * 示例：登录错误密码必须被拒绝、三权分立越权访问必须被拦截、账号锁定必须生效
  * 判断标准：如果该用例不通过，版本不应继续进入详细测试或发布验证
  * 🔥 **SMOKE 用例的 priority 字段必须设置为 "high"**
  
- **FULL（全量用例）**：详细功能验证、完整业务流程、非核心功能、补充场景
- **ABNORMAL（异常用例）**：非法输入、错误凭据、失败流程、容错与错误提示
- **BOUNDARY（边界用例）**：最大/最小值、空值、超长、临界条件
- **PERFORMANCE（性能用例）**：响应时间、并发、负载、吞吐量
- **SECURITY（安全用例）**：权限、认证授权、注入防护、敏感数据
- **USABILITY（可用性用例）**：易用性、交互体验、界面友好性
- **COMPATIBILITY（兼容性用例）**：浏览器/设备/平台/版本适配
- **RELIABILITY（可靠性用例）**：故障恢复、稳定性、长时间运行、降级

**分类优先级提醒：**
- 权限隔离、越权、认证授权、黑白名单、账号锁定、攻击防护 → 优先使用 SECURITY
- 响应时间、并发、负载、长时间运行性能指标 → 优先使用 PERFORMANCE
- 浏览器、分辨率、平台版本、数据库类型适配 → 优先使用 COMPATIBILITY
- 交互体验、页面提示、文案友好性、操作易用性 → 优先使用 USABILITY
- 故障恢复、主备切换、中断恢复、备份恢复、服务降级 → 优先使用 RELIABILITY
- 普通非法输入、错误提示、失败流程 → 使用 ABNORMAL
- 密码/验证码的长度、位数、Min/Max/空值、阈值 → 使用 BOUNDARY，不要因为属于登录安全策略而改成 SECURITY
- 密码复杂度字符类型不足、确认密码不一致 → 使用 ABNORMAL；若同时命中明确长度/Min/Max 边界，则使用 BOUNDARY
- 正常但非冒烟的详细功能验证 → 使用 FULL

## 重要提示

### 用例数量要求（重要！）
- **以用户提示中的推荐数量为准**，不要套用固定 1-3 条。
- **只有极其简单的测试点才生成1个用例**（如：纯查看类、单一操作）。
- **复杂策略/边界测试点必须生成多条用例**，以覆盖：
  * 正常情况 vs 异常情况
  * 有数据 vs 无数据
  * 不同的输入组合
  * 不同的边界值
...
const rawEstimatedCount = Number(testPoint.estimatedTestCases || 0);
const isPasswordComplexityNegativePoint =
  /密码复杂度|复杂度不满足|不满足复杂度|不符合复杂度|密码.*不符合/.test(
    `${testPoint.testPoint}\n${testPoint.description || ''}\n${testPoint.steps || ''}\n${testPoint.expectedResult || ''}`
  );
const recommendedCaseCount = isPasswordComplexityNegativePoint
  ? Math.max(8, Math.min(10, Math.round(rawEstimatedCount) || 8))
  : Number.isFinite(rawEstimatedCount) && rawEstimatedCount > 0
    ? Math.max(1, Math.min(10, Math.round(rawEstimatedCount)))
    : 2;
const recommendedCaseCountRange = isPasswordComplexityNegativePoint ? '8-10' : `1-${Math.max(3, recommendedCaseCount)}`;
const passwordComplexityCaseGuidance = isPasswordComplexityNegativePoint
  ? `
### 🔐 密码策略/复杂度专项要求
- 该测试点属于组合型密码策略校验，必须生成 **8-10 条** 用例。
- 必须优先依据「相关需求内容」中的 FR-074、FR-C-001，不得把通用经验当需求。
- 如果需求中存在最小长度、最大长度，必须覆盖完整边界：
  1. Min-1：低于最小长度，预期失败
  2. Min：等于最小长度且字符类型满足策略，预期成功
  3. Max：等于最大长度且字符类型满足策略，预期成功
  4. Max+1：超过最大长度，预期失败
- 如果需求中存在低/中/高复杂度等级，必须按等级生成代表用例：
  5. 低复杂度：满足最低长度和任意1类字符，预期成功
  6. 中复杂度：满足最低长度和任意2类字符，预期成功
  7. 高复杂度：满足最低长度和任意3类字符，预期成功
  8. 高复杂度字符类型不足（少于3类），预期失败
- 注意：若需求写的是“数字、大写字母、小写字母、特殊符号中的任意三种”，不要错误要求必须包含特殊字符；只要达到任意三类即可通过。
- 如果测试点名称包含“不满足”，仍要生成 Min/Max 等正向边界用例，用于证明边界规则正确；caseType 可用 BOUNDARY/FULL，不要全部写成 ABNORMAL。
`
  : '';
...
### 3. 用例数量要求（关键！）
🔥 **请为该测试点生成 ${recommendedCaseCount} 个测试用例（允许范围 ${recommendedCaseCountRange} 个）**：
   - **只生成与当前测试点直接相关的用例**
   - **不要生成与测试点无关的正常流程用例**
   - 例如：
   ...
   - **特殊情况**：如果测试点本身非常简单，可降到1个；如果涉及多个数据组合，可提升到3个

${passwordComplexityCaseGuidance}
...
⚠️ **SMOKE 不等于只测正向流程。主流程和高风险异常/安全门禁都可以是 SMOKE。**
...
**高风险异常/安全门禁也应生成 SMOKE 的情况：**
- 认证失败必须拦截：如错误密码、错误验证码、双因子验证码错误
- 权限边界必须拦截：如三权分立越权访问、黑白名单命中
- 安全策略必须生效：如账号锁定、强制改密拦截、阻断规则
- 可靠性门禁必须通过：如主备切换、恢复、升级回退基本成功

**不建议生成 SMOKE 的情况：**
- 普通字段空值、普通长度 Min/Max 枚举、页面文案、低风险展示类
- 一个测试点生成多条边界/异常用例时，只选择最关键的 1 条作为 SMOKE，其余保持 BOUNDARY/ABNORMAL/SECURITY/FULL

**用例类型判断规则：**
- 主流程/正常流程 → 至少 1 条 SMOKE，priority 为 high
- 高风险异常/安全门禁 → 可生成 1 条 SMOKE，priority 为 high，其余用例按真实性质分类
- 边界条件（长度、数量、阈值、Min/Max/空值、超长/过短） → BOUNDARY
- 普通异常情况 → ABNORMAL
- 测试点涉及性能/响应/并发 → PERFORMANCE
- 测试点涉及安全/权限/注入/越权/锁定/攻击防护 → SECURITY
- 测试点涉及易用性/体验/界面 → USABILITY
- 测试点涉及兼容/浏览器/设备 → COMPATIBILITY
- 测试点涉及可靠/恢复/稳定 → RELIABILITY
- 需要完整覆盖但非上述专项 → FULL

**排序规则（已由系统自动处理，无需关注）：**
1. 系统会自动将 SMOKE 类型用例排在前面
2. 随后按 FULL、BOUNDARY、ABNORMAL 及专项类型排序
...
- [ ] 如果测试点是主流程或高风险门禁，是否至少有 1 条 SMOKE？
- [ ] 如果测试点是边界/异常/性能/安全等，caseType 是否与专项类型匹配？
- [ ] 用例名称是否与测试点名称直接对应？

### 6. 冒烟用例要求
⚠️ **当测试点是主流程或高风险门禁时，必须生成 1 条 SMOKE 类型的冒烟用例**
   - 如果测试点是"用户名和密码均正确" → 生成 SMOKE 用例
   - 如果测试点是"密码错误且必须拒绝登录" → 可生成 SMOKE 用例
   - 如果测试点是"三权分立越权访问被拦截" → 可生成 SMOKE 用例
   - 如果测试点是普通"用户名为空" → 通常生成 BOUNDARY，不必 SMOKE
...
请基于测试点的步骤和预期结果，**生成 ${recommendedCaseCount} 个与当前测试点直接对应的测试用例（允许 ${recommendedCaseCountRange} 个）**。

🔥 **关键要求：**
1. **用例必须与测试点直接对应** - 不要生成无关的正常流程用例
2. **用例名称必须反映测试点的核心场景** - 例如测试点是"用户名为空"，用例名也应该是"用户名为空"相关
3. **根据测试点性质选择正确的用例类型**（caseType 必须为 ${CASE_TYPE_JSON_ENUM} 之一）：
   - 正常流程 → SMOKE
   - 边界条件 → BOUNDARY
   - 异常情况 → ABNORMAL
   - 性能相关 → PERFORMANCE
...
直接输出JSON格式，不要其他说明文字。`;
    ...
    try {
      ...
      const testCaseMaxTokens = recommendedCaseCount > 6 ? 9000 : recommendedCaseCount > 3 ? 6500 : 4000;
      const aiResponse = await this.callAI(systemPrompt, userPrompt, testCaseMaxTokens);
      ...
      testCases.forEach(tc => {
        ...
        this.normalizeCaseType(tc, testPoint);
      ...
      // 🔥 智能判断：主流程和高风险异常/安全门禁都可以需要 SMOKE 用例
      const shouldHaveSmokeCase = this.shouldHaveSmokeCase(testPoint);
      const hasSmokeCase = testCases.some(tc => tc.caseType === 'SMOKE');
      
      if (shouldHaveSmokeCase && !hasSmokeCase && testCases.length > 0) {
        console.warn('⚠️  警告：关键门禁测试点没有生成冒烟用例，自动选择代表性用例转换为 SMOKE 类型');
        
        const targetCase = this.selectSmokeCandidateForTestPoint(testCases, testPoint) || testCases[0];
        targetCase.caseType = 'SMOKE';
        targetCase.priority = 'high'; // 🔥 冒烟用例优先级必须为 high
        
        console.log(`   ✅ 已将用例 "${targetCase.name}" 设置为 SMOKE（冒烟用例），优先级：high`);
      } else if (!shouldHaveSmokeCase && hasSmokeCase) {
        console.warn('⚠️  警告：非关键门禁测试点生成了冒烟用例，自动调整用例类型');
      ...
      if (filteredCasesWithMark.length > 0) {
        console.warn('🔄 存在被过滤用例，自动带过滤原因补生成一次...');
        const retryPrompt = `${userPrompt}

## 🚫 上一次生成结果中存在被过滤用例，请只补生成这些被过滤用例
${filteredCasesWithMark.map((tc, index) => `
### 被过滤用例 ${index + 1}
- 用例名称: ${tc.name}
- 过滤原因: ${(tc as any).filterReason}
- 测试数据: ${tc.testData || '无'}
- 操作步骤: ${tc.steps || '无'}
`).join('\n')}

## 本次重试要求
- 只修复数据一致性问题，不要偏离当前测试点。
- 只生成 ${filteredCasesWithMark.length} 个替代用例，用于替换上面被过滤的用例。
- 用例名称、testData、steps 必须逐字对应。
- 如果用例名称说输入某个值，steps 中必须明确输入该值，testData 中必须列出该值。
- 如果用例名称说某字段为空，testData 必须标记为“（空）”，steps 必须说明该字段保持为空。

直接输出JSON格式，不要其他说明文字。`;

        try {
          const retryResponse = await this.callAI(systemPrompt, retryPrompt, 4000);
          const retryParsed = this.parseAIJsonResponse(retryResponse, '测试用例自动重试生成');
          const retryCases: TestCase[] = retryParsed.testCases || [];

          retryCases.forEach(tc => {
            tc.system = systemName;
            tc.module = moduleName;
            tc.testScenario = scenarioName;
            tc.testScenarioId = scenarioId;
          ...
          const retryShouldHaveSmokeCase = this.shouldHaveSmokeCase(testPoint);
          const retryHasSmokeCase = retryCases.some(tc => tc.caseType === 'SMOKE');
          if (retryShouldHaveSmokeCase && !retryHasSmokeCase && retryCases.length > 0) {
            const targetCase = this.selectSmokeCandidateForTestPoint(retryCases, testPoint) || retryCases[0];
            targetCase.caseType = 'SMOKE';
            targetCase.priority = 'high';
          } else if (!retryShouldHaveSmokeCase && retryHasSmokeCase) {
            retryCases.forEach(tc => {
              if (tc.caseType === 'SMOKE') {
                const isAbnormal = /错误|失败|异常|无效/.test(testPoint.testPoint);
                tc.caseType = isAbnormal ? 'ABNORMAL' : 'BOUNDARY';
          ...
          const retryValidCases: TestCase[] = [];
          const retryInvalidCases: { case: TestCase; validation: any }[] = [];
          console.log('\n🔍 [数据一致性验证] 检查自动重试生成的测试用例...');
          retryCases.forEach((tc, index) => {
            const validation = this.validateTestCaseConsistency(tc);
            if (!validation.isValid && validation.severity === 'error') {
              retryInvalidCases.push({ case: tc, validation });
              console.error(`   🚫 自动重试用例 ${index + 1} 仍存在严重错误: ${validation.warnings.join('; ')}`);
            } else {
              retryValidCases.push(tc);
              console.log(`   ✅ 自动重试用例 ${index + 1}: "${tc.name}" - 数据一致性检查通过`);
            }
          });

          retryValidCases.sort((a, b) => {
            const isMainFlowA = this.isMainFlowTestCase(a.name);
            const isMainFlowB = this.isMainFlowTestCase(b.name);
            if (isMainFlowA && !isMainFlowB) return -1;
            if (!isMainFlowA && isMainFlowB) return 1;
            const typeA = CASE_TYPE_SORT_ORDER[a.caseType || 'FULL'] || 99;
            const typeB = CASE_TYPE_SORT_ORDER[b.caseType || 'FULL'] || 99;
          ...
          if (retryValidCases.length > 0) {
            const retryFilteredCasesWithMark = retryInvalidCases.map(({ case: tc, validation }) => ({
              ...tc,
              isFiltered: true,
              filterReason: validation.warnings.join('; ')
            }));
            const combinedValidCases = [...finalTestCases, ...retryValidCases];
            combinedValidCases.sort((a, b) => {
              const isMainFlowA = this.isMainFlowTestCase(a.name);
              const isMainFlowB = this.isMainFlowTestCase(b.name);
              if (isMainFlowA && !isMainFlowB) return -1;
              if (!isMainFlowA && isMainFlowB) return 1;
            ...
            console.log(`✅ 自动补生成成功，新增 ${retryValidCases.length} 个有效用例${retryFilteredCasesWithMark.length > 0 ? `，仍过滤 ${retryFilteredCasesWithMark.length} 个` : ''}`);
            return {
              validCases: combinedValidCases,
              filteredCases: retryFilteredCasesWithMark,
              totalGenerated: testCases.length + retryCases.length,
              validCount: combinedValidCases.length,
              filteredCount: retryFilteredCasesWithMark.length
            };
          }

          console.error('🚫 自动补生成后仍没有有效替代用例，将保留原有效用例并返回过滤信息');
          filteredCasesWithMark.push(...retryInvalidCases.map(({ case: tc, validation }) => ({
            ...tc,
            isFiltered: true,
            filterReason: validation.warnings.join('; ')
          })));
        } catch (retryError: any) {
          console.error('❌ 自动重试生成测试用例失败:', retryError.message);
        }
      }
  ...
  private determineCaseType(tc: any, testPoint: TestPoint): string {
    ...
    if (this.containsKeywords(combinedText, ['性能', '响应时间', '并发', '吞吐量', '负载', '压力测试', '耗时', '处理速率'])) {
      return 'PERFORMANCE';
    }
    
    // 2. 边界用例 - 长度/数量/阈值类应优先于安全策略泛匹配
    if (this.isBoundaryCaseText(combinedText)) {
      return 'BOUNDARY';
    }

    // 3. 兼容性用例 - 关键词：兼容、浏览器、设备、平台、版本
    if (this.containsKeywords(combinedText, ['兼容', '浏览器', 'chrome', 'firefox', 'edge', '分辨率', '设备', '平台', '版本', '适配', '多端', '数据库类型'])) {
      return 'COMPATIBILITY';
    }

    // 4. 可用性用例 - 关键词：可用性、易用、友好、体验、交互
    if (this.containsKeywords(combinedText, ['可用性', '易用', '友好', '体验', '交互', 'ui', 'ux', '界面', '文案', '提示信息', '布局'])) {
      return 'USABILITY';
    }

    // 5. 可靠性用例 - 关键词：可靠、恢复、容错、降级、稳定
    if (this.containsKeywords(combinedText, ['可靠', '恢复', '容错', '降级', '宕机', '中断恢复', '稳定性', '主备切换', '故障恢复', '备份恢复', '回退'])) {
      return 'RELIABILITY';
    }

    // 6. 安全用例 - 关键词：权限隔离、认证授权、攻击防护等安全门禁
    if (this.containsKeywords(combinedText, [
      '安全', '权限', '权限隔离', '越权', '三权分立', '加密', '认证', '授权',
      'xss', 'sql注入', '防护', '攻击', '黑名单', '白名单',
      '账号锁定', '双因子', '强制改密拦截', '未完成改密', '会话劫持',
      '暴力破解', '破解防护', '加密传输', '明文传输', 'https',
      '会话超时', '会话失效', '受保护页面'
    ])) {
      return 'SECURITY';
    }

    // 7. 异常用例 - 关键词：异常、错误、失败、非法、无效、中断
    if (this.containsKeywords(combinedText, [
      '异常', '错误', '失败', '非法', '无效', '中断', '故障',
      '不一致', '不符合', '不满足', '复杂度不足', '缺少', '仅含', '纯数字'
    ])) {
      return 'ABNORMAL';
    }
  ...
  private normalizeCaseType(tc: TestCase, testPoint: TestPoint): void {
    const inferredType = this.determineCaseType(tc, testPoint);
    const currentType = (tc.caseType || '').toUpperCase();
    const validTypes = new Set(CASE_TYPE_JSON_ENUM.split('|'));

    if (currentType === 'SMOKE') {
      tc.caseType = 'SMOKE';
    } else if (inferredType === 'BOUNDARY') {
      tc.caseType = 'BOUNDARY';
    } else if (!validTypes.has(currentType)) {
      tc.caseType = inferredType;
    } else if (
      currentType === 'ABNORMAL' || currentType === 'BOUNDARY' || currentType === 'FULL'
    ) {
      if (['PERFORMANCE', 'SECURITY', 'USABILITY', 'COMPATIBILITY', 'RELIABILITY'].includes(inferredType)) {
        tc.caseType = inferredType;
      } else {
        tc.caseType = currentType;
      }
    } else {
      tc.caseType = currentType;
    }
  ...
  private isBoundaryCaseText(text: string): boolean {
    const normalizedText = text.toLowerCase();
    const boundaryKeywords = [
      '边界', '最大', '最小', '极限', '超出', '超长', '过短', '范围',
      '限制', '上限', '下限', '阈值', '临界', '为空', '空值', '留空',
      '未输入', '不输入', '不填', '长度', '位数', '字符数',
      'min', 'max', 'min-1', 'max+1', '等于最小', '等于最大',
      '低于最小', '超过最大', '小于最小', '大于最大'
    ];

    if (this.containsKeywords(normalizedText, boundaryKeywords)) {
      return true;
    }

    return /\d+\s*(位|字符|字节|条|个|次|分钟|秒|天)/.test(normalizedText);
  }

  private shouldHaveSmokeCase(testPoint: TestPoint): boolean {
    const text = [
      testPoint.testPoint,
      testPoint.testPointType || '',
      testPoint.description || '',
    ...
    if (testPoint.testPointType === 'main' || this.isMainFlowTestCase(testPoint.testPoint)) {
      return true;
    }

    const isHighRisk = testPoint.riskLevel === 'high' || /高风险|核心|必须|关键|门禁/.test(text);
    const isCriticalNegativeGate = /错误密码|密码错误|验证码错误|双因子|账号锁定|登录失败|锁定阈值|权限隔离|越权|无权限|黑名单|白名单|阻断|拦截|强制改密|未完成改密|证书|许可证|主备切换|恢复|升级回退|备份恢复|告警/.test(text);

    return isHighRisk && isCriticalNegativeGate;
  }

  private shouldPromoteCaseToSmoke(tc: TestCase): boolean {
    const text = [
      tc.name,
      tc.description || '',
      tc.testPurpose || '',
    ...
    if (this.isMainFlowTestCase(tc.name)) {
      return true;
    }

    const isHighRisk = tc.priority === 'high' || /高风险|核心|必须|关键|门禁/.test(text);
    const isCriticalNegativeGate = /错误密码|密码错误|验证码错误|双因子|账号锁定|登录失败|锁定阈值|权限隔离|越权|无权限|黑名单|白名单|阻断|拦截|强制改密|未完成改密|证书|许可证|主备切换|恢复|升级回退|备份恢复|告警/.test(text);

    return isHighRisk && isCriticalNegativeGate;
  }

  private selectSmokeCandidateForTestPoint(testCases: TestCase[], testPoint: TestPoint): TestCase | undefined {
    const pointText = [
      testPoint.testPoint,
      testPoint.description || '',
      testPoint.coverageAreas || '',
    ...
    const isPasswordComplexityNegativePoint =
      /密码复杂度|复杂度不满足|不满足复杂度|不符合复杂度|密码.*不符合|密码策略/.test(pointText);

    if (isPasswordComplexityNegativePoint) {
      const isNonBoundaryComplexityCase = (tc: TestCase) => {
        const text = `${tc.name}\n${tc.description || ''}\n${tc.steps || ''}\n${tc.assertions || ''}`;
        const isCharacterClassFailure =
          /纯数字|仅含数字|仅包含数字|字符类型不足|少于三类|少于3类|缺少.*(大写|小写|数字|特殊|字符)/.test(text);
        const isLengthBoundary =
          /短于|低于|超过|超出|最大值|最小值|最大长度|最小长度|min|max/.test(text);
        const isConfirmationMismatch = /确认密码|两次密码|不一致/.test(text);
        return isCharacterClassFailure && !isLengthBoundary && !isConfirmationMismatch;
      };

      return testCases.find(isNonBoundaryComplexityCase)
        || testCases.find(tc => /短于|低于最小|小于最小/.test(tc.name))
        || testCases.find(tc => /超过最大|大于最大|超出最大/.test(tc.name));
    }

    return testCases.find(tc => this.isMainFlowTestCase(tc.name))
      || testCases.find(tc => this.shouldPromoteCaseToSmoke(tc));
  }
  ...
  private normalizeDetectedFieldName(field: string): string {
    let normalized = (field || '').trim();
    normalized = normalized.replace(/^(错误|正确|有效|无效|异常|非法)/, '');
    normalized = normalized.replace(/(不一致|不匹配|不符合|不满足|一致|匹配)$/, '');
    normalized = normalized.replace(/^(输入框|框|字段|表单)/, '');
    normalized = normalized.replace(/(输入框|字段|框)$/, '');

    const knownFields = ['确认新密码', '确认密码', '新密码', '旧密码', '原密码', '用户名', '密码', '验证码', '账号'];
    const matchedKnownField = knownFields.find(knownField => normalized.includes(knownField));
    return matchedKnownField || normalized;
  }

  private isLikelyInputField(field: string): boolean {
    const normalized = (field || '').trim();
    if (!normalized) return false;

    const knownFields = ['确认新密码', '确认密码', '新密码', '旧密码', '原密码', '用户名', '密码', '验证码', '账号'];
    if (knownFields.includes(normalized)) return true;

    const nonFieldFragments = [
      '输入', '填写', '选择', '点击', '打开', '跳转', '进入',
      'sql', 'xss', '注入', '载荷', 'payload', '联合查询', '注释符', '特殊字符',
      '基本', '防护', '验证', '拦截', '攻击', '脚本', '语句'
    ];
    if (nonFieldFragments.some(fragment => normalized.includes(fragment))) {
      return false;
    }

    const fieldLikeSuffixes = ['名', '码', '号', '字段', '类型', '地址', '端口', '名称', '路径', '时间', '邮箱', '手机号'];
    return fieldLikeSuffixes.some(suffix => normalized.endsWith(suffix));
  }
  ...
  private validateTestCaseConsistency(testCase: TestCase): { isValid: boolean; warnings: string[]; severity: 'error' | 'warning' | 'ok' } {
    ...
    fieldExtractionPatterns.forEach(pattern => {
      ...
      for (const match of matches) {
        const field = this.normalizeDetectedFieldName(match[1]);
        ...
        const excludeWords = [
          ...
          '输入', '填写', '选择', '点击', '打开', '进入', '跳转',
          // 规则描述词（不是输入字段）
          '复杂度', '复杂度要求', '不符合', '不满足', '符合要求', '安全策略', '密码策略'
        ];
        const isValidField = field && 
                            field.length >= 2 && 
                            field.length <= 8 && 
                            this.isLikelyInputField(field) &&
                            !excludeWords.includes(field) &&
                            !field.includes('要求') &&
                            !field.includes('规则') &&
                            !field.includes('复杂度') &&
                            !field.startsWith('不符合') &&
                            !field.startsWith('不满足') &&
                            !field.includes('和') &&
                            !field.includes('或');
    ...
    for (const match of dataMatches) {
      ...
      if (field && value && !value.includes('空') && value.length > 0) {
        const normalizedField = this.normalizeDetectedFieldName(field);
        const isDynamicCaptchaValue =
          normalizedField === '验证码' &&
          (value.includes('正确验证码') || value.includes('当前验证码') || value.includes('当前显示') || value.includes('动态验证码'));
        if (isDynamicCaptchaValue) {
          continue;
        }
...
6. ⚠️ **如果原用例包含主流程或高风险门禁，优化后必须保留或生成 1 条 caseType="SMOKE" 的冒烟用例**
...
## 🔥 用例类型要求
- **当存在主流程或高风险门禁时，必须确保至少有 1 条 SMOKE（冒烟用例）**
- SMOKE 用例用于验证版本是否具备最小可验收能力，不限于正向流程
- 登录失败必须拦截、越权必须阻断、账号锁定必须生效等高风险异常/安全门禁也可以是 SMOKE
- 如果原始用例中有 SMOKE 类型，必须保留
- 如果原始用例中没有 SMOKE 类型，仅在存在主流程或高风险门禁时将最核心的用例设置为 SMOKE
- 普通字段空值、普通长度边界、普通错误提示，不要强制改成 SMOKE
...
\`\`\``;
    ...
    try {
      ...
      if (!hasSmokeCase && newCases.length > 0) {
        const targetCase = newCases.find(tc => this.shouldPromoteCaseToSmoke(tc));
        if (targetCase) {
          console.warn('⚠️  警告：重新生成的关键门禁用例中没有冒烟用例，自动补充 SMOKE 类型');
          targetCase.caseType = 'SMOKE';
          targetCase.priority = 'high';
          console.log(`   ✅ 已将用例 "${targetCase.name}" 设置为 SMOKE（冒烟用例），优先级：high`);
        }
      ...
      // 🔥 仅当存在主流程或高风险门禁用例时，才自动补充 SMOKE（回退方案）
      const hasSmokeCase = newCases.some(tc => tc.caseType === 'SMOKE');
      if (!hasSmokeCase && newCases.length > 0) {
        const targetCase = newCases.find(tc => this.shouldPromoteCaseToSmoke(tc));
        if (targetCase) {
          console.warn('⚠️  回退方案：关键门禁用例缺少 SMOKE，自动补充');
          targetCase.caseType = 'SMOKE';
          targetCase.priority = 'high';
          console.log(`   ✅ 已将用例 "${targetCase.name}" 设置为 SMOKE（冒烟用例），优先级：high`);
        }
  ...
  /**
   * 从需求文档文本中自动识别字符长度/数值范围（不写死具体阈值）
   */
  private extractRangeConstraintsFromText(
    text: string
  ): Array<{ min: number; max: number; fieldName?: string }> {
    if (!text?.trim()) return [];

    const found = new Map<string, { min: number; max: number; fieldName?: string }>();
    const add = (min: number, max: number, fieldName?: string) => {
      if (!Number.isFinite(min) || !Number.isFinite(max) || min < 0 || max <= 0 || min >= max || max > 10000) {
        return;
      }
      const key = `${fieldName || ''}:${min}-${max}`;
      if (!found.has(key)) {
        found.set(key, { min, max, fieldName: fieldName?.trim() || undefined });
      }
    };
    ...
    const runPattern = (
      ...
      while ((match = re.exec(text)) !== null) {
        const min = parseInt(match[minIdx], 10);
        const max = parseInt(match[maxIdx], 10);
        const fieldName = fieldIdx !== undefined ? match[fieldIdx] : undefined;
        add(min, max, fieldName);
      }
    };

    // 字段名 + 长度 X-Y 字符
    runPattern(
      /([\u4e00-\u9fa5a-zA-Z][\u4e00-\u9fa5a-zA-Z0-9_]{0,15})?\s*长度[为是：:\s]*(\d+)\s*[-~－至到]\s*(\d+)\s*个?字符/gi,
      2,
      3,
      1
    );
    runPattern(/(?:验证)?长度[为是：:\s]*(\d+)\s*[-~－至到]\s*(\d+)\s*个?字符/gi, 1, 2);
    runPattern(/字符[数]?长度[：:\s]*(\d+)\s*[-~－至到]\s*(\d+)/gi, 1, 2);
    runPattern(/(\d+)\s*[-~－至到]\s*(\d+)\s*个?字符/gi, 1, 2);
    runPattern(/(\d+)\s*到\s*(\d+)\s*个?字符/gi, 1, 2);
    runPattern(/(\d+)\s*[~～]\s*(\d+)\s*字/gi, 1, 2);
    runPattern(/限\s*(\d+)\s*[-~－]\s*(\d+)\s*位/gi, 1, 2);
    runPattern(/最少\s*(\d+)\s*个?字符[\s\S]{0,80}?最多\s*(\d+)\s*个?字符/gi, 1, 2);
    runPattern(/至少\s*(\d+)[\s\S]{0,80}?至多\s*(\d+)\s*个?字符/gi, 1, 2);
    runPattern(/不少于\s*(\d+)\s*字[\s\S]{0,80}?不超过\s*(\d+)\s*字/gi, 1, 2);
    runPattern(/最小\s*(\d+)[\s\S]{0,60}?最大\s*(\d+)[\s\S]{0,30}?字符/gi, 1, 2);
    runPattern(/最小长度\s*(\d+)[\s\S]{0,40}?最大长度\s*(\d+)/gi, 1, 2);

    // 字段名：… X-Y字符（同行）
    runPattern(
      /([\u4e00-\u9fa5]{2,12})[：:][^\n]{0,40}?(\d+)\s*[-~－至到]\s*(\d+)\s*个?字符/gi,
      2,
      3,
      1
    );

    // 原型/表格：长度列 0/100、6/20（前后文含字符/长度/位）
    const slashRe = /(\d+)\s*\/\s*(\d+)/g;
    let slashMatch: RegExpExecArray | null;
    while ((slashMatch = slashRe.exec(text)) !== null) {
      const start = Math.max(0, slashMatch.index - 40);
      const end = Math.min(text.length, slashMatch.index + slashMatch[0].length + 40);
      const ctx = text.slice(start, end);
      if (!/字符|长度|字数|位|maxlength|minlength|输入框/i.test(ctx)) continue;
      const min = parseInt(slashMatch[1], 10);
      const max = parseInt(slashMatch[2], 10);
      const fieldBefore = text.slice(Math.max(0, slashMatch.index - 30), slashMatch.index);
      const fieldMatch = fieldBefore.match(/([\u4e00-\u9fa5]{2,12})\s*[|｜]?\s*$/);
      add(min, max, fieldMatch?.[1]);
    }

    // maxlength / minlength（HTML 或说明中的属性）
    const maxOnly = new Map<number, string | undefined>();
    const minOnly = new Map<number, string | undefined>();
    let attrMatch: RegExpExecArray | null;
    const maxAttrRe = /maxlength\s*=\s*["']?(\d+)/gi;
    while ((attrMatch = maxAttrRe.exec(text)) !== null) {
      maxOnly.set(parseInt(attrMatch[1], 10), undefined);
    }
    const minAttrRe = /minlength\s*=\s*["']?(\d+)/gi;
    while ((attrMatch = minAttrRe.exec(text)) !== null) {
      minOnly.set(parseInt(attrMatch[1], 10), undefined);
    }
    for (const max of maxOnly.keys()) {
      const mins = [...minOnly.keys()].filter(m => m < max);
      const min = mins.length > 0 ? Math.max(...mins) : 0;
      add(min, max);
    }
   ...
   * 判断测试点列表是否已覆盖某类长度边界
   */
  private hasLengthBoundaryCoverage(
    testPoints: TestPoint[],
    min: number,
    max: number,
    kind: 'below' | 'atMin' | 'atMax' | 'above'
  ): boolean {
    const belowVal = min - 1;
    const aboveVal = max + 1;

    return testPoints.some(tp => {
      const n = tp.testPoint || '';
      if (!/字符|长度|位|个字/.test(n)) return false;

      switch (kind) {
        case 'below':
          return (
            n.includes(String(belowVal)) ||
            /低于|少于|不足|小于.*最小|短于/.test(n) ||
            (n.includes(String(min)) && /低于|少于|不足|小于/.test(n))
          );
        case 'atMin':
          return (
            (n.includes(String(min)) && /最小|下限|恰好|等于.*边界|边界/.test(n)) ||
            (n.includes(`恰好${min}`) || n.includes(`等于${min}`))
          );
        case 'atMax':
          return (
            (n.includes(String(max)) && /最大|上限|恰好|等于.*边界|边界/.test(n)) ||
            (n.includes(`恰好${max}`) || n.includes(`等于${max}`))
          );
        case 'above':
          return (
            n.includes(String(aboveVal)) ||
            /超过|超出|大于.*最大|长于/.test(n) ||
            (n.includes(String(max)) && /超过|超出|大于/.test(n))
          );
        default:
          return false;
      }
   ...
   * 构建单个长度边界测试点
   */
  private buildLengthBoundaryTestPoint(
    scenarioName: string,
    fieldLabel: string,
    min: number,
    max: number,
    ...
    const belowVal = min - 1;
    const aboveVal = max + 1;
    const samples: Record<typeof kind, { name: string; steps: string; expected: string }> = {
      below: {
        name: `${fieldLabel}输入${belowVal}字符（低于最小值${min}）`,
        steps: `1. 打开相关页面或表单\n2. 在${fieldLabel}中输入${belowVal}个任意合法字符\n3. 触发校验（失焦或提交）`,
        expected: `系统提示长度不符合要求（不少于${min}字符），不允许通过校验`,
      },
      atMin: {
        name: `${fieldLabel}输入恰好${min}字符（等于最小边界）`,
        steps: `1. 打开相关页面或表单\n2. 在${fieldLabel}中输入恰好${min}个字符\n3. 触发校验（失焦或提交）`,
        expected: `校验通过，接受${min}字符输入`,
      },
      atMax: {
        name: `${fieldLabel}输入恰好${max}字符（等于最大边界）`,
        steps: `1. 打开相关页面或表单\n2. 在${fieldLabel}中输入恰好${max}个字符\n3. 触发校验（失焦或提交）`,
        expected: `校验通过，接受${max}字符输入`,
      },
      above: {
        name: `${fieldLabel}输入${aboveVal}字符（超过最大值${max}）`,
        steps: `1. 打开相关页面或表单\n2. 在${fieldLabel}中输入${aboveVal}个字符\n3. 触发校验（失焦或提交）`,
        expected: `系统提示长度超出限制（不超过${max}字符），不允许通过校验`,
      },
    };
    ...
    return {
      testScenario: scenarioName,
      testPoint: s.name,
      description: `验证${fieldLabel}在长度范围${min}-${max}字符时的${kind === 'below' ? '低于最小值' : kind === 'atMin' ? '最小边界' : kind === 'atMax' ? '最大边界' : '超过最大值'}场景`,
      coverageAreas: `${fieldLabel}长度校验`,
      estimatedTestCases: 1,
      steps: s.steps,
  ...
  /**
   * 根据需求中的长度范围，补充 AI 遗漏的边界测试点
   */
  private supplementBoundaryTestPoints(
    testPoints: TestPoint[],
    requirementText: string,
    scenarioName: string
  ): TestPoint[] {
    const constraints = this.extractRangeConstraintsFromText(requirementText);
    if (constraints.length === 0) return testPoints;

    const result = [...testPoints];
    const existingNames = new Set(result.map(tp => tp.testPoint));

    for (const { min, max, fieldName } of constraints) {
      const fieldLabel = fieldName || '字段';
      const kinds: Array<'below' | 'atMin' | 'atMax' | 'above'> =
        min > 0 ? ['below', 'atMin', 'atMax', 'above'] : ['atMax', 'above'];

      for (const kind of kinds) {
        if (this.hasLengthBoundaryCoverage(result, min, max, kind)) continue;

        const tp = this.buildLengthBoundaryTestPoint(scenarioName, fieldLabel, min, max, kind);
        if (existingNames.has(tp.testPoint)) continue;

        result.push(tp);
        existingNames.add(tp.testPoint);
        console.log(`   📏 补充边界测试点: ${tp.testPoint}`);
      }
    }
  ...
  private parseAIJsonResponse(aiResponse: string, context: string = 'JSON解析'): any {
    if (typeof aiResponse !== 'string' || aiResponse.trim().length === 0) {
      throw new Error(`${context}失败: AI响应内容为空，无法解析JSON。请检查模型是否因输出Token不足、思考模式占用输出或响应被截断而未返回最终JSON。`);
    }
