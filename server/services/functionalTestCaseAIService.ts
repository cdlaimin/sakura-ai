...
const recommendedCaseCount = Number.isFinite(rawEstimatedCount) && rawEstimatedCount > 0
  ? Math.max(1, Math.min(3, Math.round(rawEstimatedCount)))
  : 2;
...
### 3. 用例数量要求（关键！）
🔥 **请为该测试点生成 ${recommendedCaseCount} 个测试用例（允许范围 1-3 个）**：
   - **只生成与当前测试点直接相关的用例**
   - **不要生成与测试点无关的正常流程用例**
   - 例如：
...
请基于测试点的步骤和预期结果，**生成 ${recommendedCaseCount} 个与当前测试点直接对应的测试用例（允许 1-3 个）**。
